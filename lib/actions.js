...
module.exports.userFolloweds = async function(uid, proxy = false, offset = 0, limit = 100)
{
    ...
    {
        await sleep(5000)
        const ok = await module.exports.userFolloweds(uid, proxy, offset + limit)
        if(! ok)
        {
            return false
        }
...
module.exports.user = async function(uid, proxy = false) {
    ...
    try
    {
        ...
        if(! await mongodb.updateUserProcessed(uid)) return false
        return true
    }
    catch (e)
    {
...
(async function()
{
    ...
    // console.log(await module.exports.userFolloweds(1, config.proxy)) // todo: fix this guy with many many fans
